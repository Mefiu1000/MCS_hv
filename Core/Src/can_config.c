...
ResponseMessageFrame ResponseMessage[NUMBER_OF_READ_REGS] =
{
	{ //K1 and K2 current value:
		...
		.Response_RegID      = K_Value, // Address of regs which response refers
		.Response_Data1      = &Read_Data1                 // Returned data to MCU opcjonalne
	},
	{//K1 and K2 state:
		.Response_DLC        = 2u,
		// Here should be another Read_ReactionHandler for this RegID
		.Response_RegID      = K_State,
		.Response_Data1      = &Read_Data2
	},
	{//IMD and BSPD state:
		.Response_DLC        = 2u,
		// Here should be another Read_ReactionHandler for this RegID
		.Response_RegID      = IMD_State,
		.Response_Data1      = &Read_Data2
	},
...
WriteMessageFrame WriteMessage[NUMBER_OF_WRITE_REGS] =
{
	...
	{
		.Write_RegID           = Fuse_State, // Reg which should be written by MCU command
